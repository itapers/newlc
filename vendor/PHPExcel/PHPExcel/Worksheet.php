...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	*/
	 ...
	 * @return array
	 */
	public function rangeToArray($pRange = 'A1', $nullValue = null, $calculateFormulas = true, $formatData = true, $returnCellRef = false,$Rqname=array()) {
		// Returnvalue
		$returnValue = array();
			...
			for ($col = $minCol; $col != $maxCol; ++$col) {
				...
				if ($this->_cellCollection->isDataSet($col.$row)) {
					...
					if ($cell->getValue() !== null) {											
						...
						if(!empty($Rqname)){
							foreach($Rqname as $vv){
								if($vv==$cRef && strlen($cell->getValue())!=8){ //M列和O列是时间
									$returnValue[$rRef][$cRef] =   gmdate("Y-m-d H:i:s", PHPExcel_Shared_Date::ExcelToPHP($cell->getValue())); 
								}
							}
						}
					} else {
	 ...
	 * @return array
	 */
	public function toArray($nullValue = null, $calculateFormulas = true, $formatData = true, $returnCellRef = false,$Rqname=array()) {
		// Garbage collect...
		$this->garbageCollect();
		...
		// Return
		$res=$this->rangeToArray(	'A1:'.$maxCol.$maxRow,
									$nullValue, $calculateFormulas, $formatData, $returnCellRef,$Rqname);
